...
let text = ""
let Info = document.getElementById("CatInfo")

for (let i = 0; i < info.articles.length; i++) {
    const article = info.articles[i];

    text += `
        <button onclick="putData('${article.category}')" class="catergory-button">${article.category}</button>
    `;
}
...
function putData(ID) {
    ...
    const e = document.querySelector('.main-section-cat');
